...
We need molecular genetics methods to study how living organisms build, assemble and coordinate their molecular components. Those methods are often complicated, unexpected and unintuitive, because those methods are tricks that scientists have invented to study objects and processes that are too small to be seen both at the naked eyes and also at the most powerful microscopes. With molecular genetics we can study the photosynthetic molecular machineries, and learn how those machineries are build from genetic information hidden in genes and DNA.

## Accessing genetic information

One trick that we can use to access genetic information is transcriptomic analysis. To understand what is transcriptomic analysis, and how this analysis helps us studying the optimization of photosynthesis, we must first understand what in biology and in genetic is a [transcript](https://en.wikipedia.org/wiki/Messenger_RNA).
...
We can imagine the plant cell as a factory, this factory can produce many components for different kinds of engines. To produce all those goods, the cell/factory needs a central library, where it can store all the projects for the various components and the instruction about when / why / how to assemble them. In the plant cell, and also in any other cell, every project is a gene, the central library is the genome.

Let's suppose that you have a factory, and also  projects for many highly specialized machineries. At a given moment the market requires you to to produce a given number of a certain machine. First, you might want to make copies of the project for that machineries and give them to the workers, so that they can study them. It make sense to distribute temporary copies of the project, instead of making everybody work on the same, single, precious copy that you keep in the library. For many reasons:
...
- Those copies are temporary, so workers can write notes on them, rip useless pages, modify them on the run, get them dirty and in the end, when they are ruined, throw them away.
...
Summing up, every cell can be seen as a factory: it has a genome, made of DNA, where it stores all the projects and instructions and information. When the cell needs to produce a machinery from those projects, it copies the projects and instructions of all its components and distributes them around the cell, so that those components can be built and assembled. The number of the copies that are distributed are roughly proportional to the number of models of that machinery that have to be produced. Those copies are made of RNA and are called transcripts.

### Get the transcripts

Now, cells are really small, and their molecular machineries are even smaller, we can not walk around a cell and see what is going on, what it is producing, like we would do in a factory. We can extract the machineries and try to study them, but they are even smaller, when we touch them they break, they fall apart, they change their behaviour.

We can access the library that stores the project: the genome, and we can learn all the components that a cell could produce, but reading and decyphering the genome is hard, which component is being produced at a given moment, for example in the leaf, in a photosynthetic cell? At what rate?
