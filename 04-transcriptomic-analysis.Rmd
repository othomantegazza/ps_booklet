...
To study how living organisms use information hidden in their genes to build, assemble and coordinate their molecular components, we use molecular genetics methods. Those methods are often complicated, unexpected and unintuitive, because those methods are tricks that scientists have invented to study objects and processes that are too small to be seen both at the naked eyes and also at the most powerful microscopes.

With molecular genetics we can study the photosynthetic molecular machineries, and learn how those machineries are build from genetic information hidden in genes and DNA.
...
Summing up, every cell can be seen as a factory: it has a genome, made of DNA, where it stores all the projects and instructions and information. When the cell needs to produce a molecular machinery, the projects for the component of that molecular machinery are copied from the genome and distributed within the cell itself, so that that machinery can be built and assembled. The number of the copies that circulate around a cell are roughly proportional to the number of units of that machinery that must to be produced. Those copies are made of RNA and are called transcripts.
